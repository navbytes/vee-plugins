...
def scan_home(home, budget_s):
    """Returns [(kb, name), ...] for each top-level, non-hidden entry, or
    None if the scan didn't finish within budget_s — mirrors the bash
    original discarding ALL partial results on a timeout (the caller only
    trusts a scan whose `run_with_timeout` exit code was 0)."""
...
sizes = scan_home(HOME, SCAN_BUDGET_S)
...
if folder_rows:
    ...
    sub.item("Scan skipped (slow disk)", color="gray")
